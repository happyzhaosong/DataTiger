...
public class JsonTool extends BaseTool {
	...
	public static String getJsonString(JsonDTO jsonDto) throws Exception
	{
		...
		jsonObj.put(GeneralConstants.JSON_SUCCESS, jsonDto.isSuccess());
		jsonObj.put(GeneralConstants.JSON_EXIST, jsonDto.isExist());
		jsonObj.put(GeneralConstants.JSON_CODE, jsonDto.getCode());
		jsonObj.put(GeneralConstants.JSON_MESSAGE, jsonDto.getMessage());		
				...
				if(valList!=null)
				{
					if(itemCount==0 && valList.size()>0)
					{
						...
						if(obj instanceof BasePageDTO)
						{
							...
							{
								jsonObj.put(GeneralConstants.JSON_TOTAL_RESULT_COUNT, pageDto.getTotalRecordsCountInThisSearch());		
							}
	...
	{	
		...
		if(!StringTool.isEmpty(jsonStr))
		{
			...
			JSONArray dtoArray = (JSONArray)jsonObj.get(GeneralConstants.JSON_ROOT_ACTIVE_MQ_MESSAGE);
