...
public class ClassTool extends BaseTool {
	...
	{
		return getMethodByPrefix(fieldName, Constants.CLASS_METHOD_PREFIX_SET);
	}
	
	public static String getGetMethod(String fieldName, String fieldType) throws Exception
	{
		String methodPrefix = "";
		if(Constants.CLASS_TYPE_BOOLEAN.equalsIgnoreCase(fieldType))
		{
			methodPrefix = Constants.CLASS_METHOD_PREFIX_IS;
		}else
		{
			methodPrefix = Constants.CLASS_METHOD_PREFIX_GET;
		}
	...
	{
		Class ret = null;
		if(Constants.CLASS_TYPE_STRING.equals(type))
		{
			ret = String.class;			
		}else if(Constants.CLASS_TYPE_INT.equals(type))
		{
			ret = int.class;
		}else if(Constants.CLASS_TYPE_LONG.equals(type))
		{
			ret = long.class;
		}else if(Constants.CLASS_TYPE_DOUBLE.equals(type))
		{
			ret = double.class;
		}else if(Constants.CLASS_TYPE_BOOLEAN.equals(type))
		{
			ret = boolean.class;
		}
		return ret;
	...
	public static void invokeSetMethod(Object obj, Field field, String value) throws Exception
	{
		...
		try
		{
			...
			}else if(Constants.CLASS_TYPE_INT.equals(fieldType))
			{
				method.invoke(obj, StringTool.getIntegerValInStr(value));				
			}else if(Constants.CLASS_TYPE_LONG.equals(fieldType))
			{
				method.invoke(obj, StringTool.getLongValInStr(value));				
			}else if(Constants.CLASS_TYPE_DOUBLE.equals(fieldType))
			{				
				method.invoke(obj, StringTool.getDoubleValInStr(value));				
			}else if(Constants.CLASS_TYPE_BOOLEAN.equals(fieldType))
			{				
	...
	public static Object invokeGetMethod(Object obj, Field field) throws Exception
	{
		...
		{
			fieldName = field.getName();
			if(!Constants.FIELD_NAME_SERIAL_VERSION_UID.equalsIgnoreCase(fieldName))
			{				
	...
	public static String getObjFieldStringVal(Object obj, Field field) throws Exception
	{
		...
		try
		{
			...
			{				
				...
				if(Constants.CLASS_TYPE_STRING.equals(fieldType) || 
				   Constants.CLASS_TYPE_INT.equals(fieldType) ||
				   Constants.CLASS_TYPE_LONG.equals(fieldType) ||
				   Constants.CLASS_TYPE_DOUBLE.equals(fieldType) ||
				   Constants.CLASS_TYPE_BOOLEAN.equals(fieldType)
				)
				{
	...
	private static Object extractValue(Class objClass, Object dataSrc) throws Exception
	{
		...
		if(fieldArr!=null)
		{
			...
			for(int i=0;i<len;i++)
			{
				...
				{
					value = WebTool.getStringAttributeBeforeParameter(fieldName, (HttpServletRequest)dataSrc);	
				...
				if(Constants.CLASS_TYPE_STRING.equals(fieldType) || Constants.CLASS_TYPE_INT.equals(fieldType) || Constants.CLASS_TYPE_LONG.equals(fieldType) || Constants.CLASS_TYPE_DOUBLE.equals(fieldType) || Constants.CLASS_TYPE_BOOLEAN.equals(fieldType))
				{
	...
	public static Object getObjValueFromJSONObject(Class objClass, JSONObject jsonObj) throws Exception
	{
		...
		if(!ClassTool.isNullObj(fieldArr))
		{
			...
			for(int i=0;i<len;i++)
			{
				...
				if(!Constants.SERIAL_VERSION_UID.equalsIgnoreCase(fieldName))
				{
					String value = "";
					try
					{
