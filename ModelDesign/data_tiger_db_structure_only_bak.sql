...
/*Table structure for table `data_sina_wei_bo` */

DROP TABLE IF EXISTS `data_sina_wei_bo`;

CREATE TABLE `data_sina_wei_bo` (
  `id` bigint(15) NOT NULL AUTO_INCREMENT,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='sina weibo crawler data table';
...
CREATE TABLE `data_xiu_hao_chi` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `shop_name` varchar(200) CHARACTER SET utf8 DEFAULT NULL COMMENT '店铺名称',
  `shop_url` text CHARACTER SET utf8 COMMENT '店铺链接地址',
  `shop_level` varchar(100) COLLATE utf8_bin DEFAULT NULL COMMENT '店铺评级，皇冠数',
  `biao_ti` varchar(1000) COLLATE utf8_bin DEFAULT NULL COMMENT '商品标题',
  `jia_ge` varchar(200) COLLATE utf8_bin DEFAULT NULL COMMENT '商品价格',
  `zhong_liang` varchar(200) COLLATE utf8_bin DEFAULT NULL COMMENT '商品净含量',
  `dan_jia` varchar(200) COLLATE utf8_bin DEFAULT NULL COMMENT '商品单价',
  `cu_xiao_jia` varchar(200) COLLATE utf8_bin DEFAULT NULL COMMENT '促销价',
  `ping_fen` varchar(100) COLLATE utf8_bin DEFAULT NULL COMMENT '商品评分',
  `img_url` varchar(2000) COLLATE utf8_bin DEFAULT NULL COMMENT '图片链接url',
  `meta_search_keyword` varchar(2000) COLLATE utf8_bin DEFAULT NULL COMMENT '商品 meta keywords 关键词信息, example: <meta name="keywords"',
  `meta_desc` varchar(2000) COLLATE utf8_bin DEFAULT NULL COMMENT '商品 meta description 描述信息, example: <meta name="description"',
  `item_url` text COLLATE utf8_bin COMMENT '商品url链接地址',
  `shang_pin_lai_yuan` tinyint(4) DEFAULT NULL COMMENT '商品来源，1---天猫， 2---淘宝， 3---京东',
  `download_task_page_url` text COLLATE utf8_bin,
  `download_task_id` bigint(20) DEFAULT NULL,
  `download_task_level` int(11) DEFAULT NULL,
  `download_task_data_parse_time` varchar(20) COLLATE utf8_bin DEFAULT NULL,
  `download_task_data_parse_time_number` bigint(100) DEFAULT NULL,
  `download_task_useless_content_page` tinyint(1) DEFAULT NULL,
  PRIMARY KEY (`id`)
) ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='保存互联网抓取的巧克力的信息';

/*Table structure for table `data_zhao_pin` */

DROP TABLE IF EXISTS `data_zhao_pin`;

CREATE TABLE `data_zhao_pin` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT,
  `job_title` varchar(1000) DEFAULT NULL COMMENT '职位名称',
  `job_update_time` varchar(200) DEFAULT NULL COMMENT '更新时间',
  `job_jing_yan` varchar(100) DEFAULT NULL COMMENT '工作年限或者经验',
  `job_area` varchar(200) DEFAULT NULL COMMENT '工作地点',
  `job_xue_li` varchar(200) DEFAULT NULL COMMENT '学历要求',
  `job_salary` varchar(1000) DEFAULT NULL COMMENT '工资待遇',
  `job_desc` text COMMENT '职位描述',
  `job_url` varchar(2000) DEFAULT NULL COMMENT '工作详情网址',
  `job_xing_bie` varchar(10) DEFAULT NULL COMMENT '职位性别要求',
  `job_browse_count` varchar(20) DEFAULT NULL COMMENT '本职位浏览次数',
  `job_jian_li_tou_di_liang` varchar(10) DEFAULT NULL COMMENT '简历投递量',
  `job_jing_zheng_li` varchar(20) DEFAULT NULL COMMENT '职位竞争力',
  `company` varchar(1000) DEFAULT NULL COMMENT '公司名称',
  `company_desc` text COMMENT '公司介绍',
  `company_gui_mo` varchar(200) DEFAULT NULL COMMENT '公司规模',
  `company_xing_zhi` varchar(100) DEFAULT NULL COMMENT '公司性质',
  `company_hang_ye` varchar(500) DEFAULT NULL COMMENT '公司行业',
  `company_web_site` varchar(2000) DEFAULT NULL COMMENT '公司网址',
  `company_zhi_zhao` varchar(10) DEFAULT NULL COMMENT '是否已经验证公司营业执照',
  `company_cheng_xin` varchar(10) DEFAULT NULL COMMENT '公司诚信等级',
  `contact_address` varchar(2000) DEFAULT NULL COMMENT '联系地址',
  `contact_telphone` varchar(100) DEFAULT NULL COMMENT '联系电话',
  `contact_email` varchar(100) DEFAULT NULL COMMENT '联系电邮',
  `contact_qq` varchar(20) DEFAULT NULL COMMENT '联系qq',
  `contact_person` varchar(200) DEFAULT NULL COMMENT '联系人',
  `download_task_page_url` text COMMENT '本条记录解析的页面的 url 链接, 每个数据表都有这个字段，便于分析解析不成功的情况',
  `download_task_data_parse_time` varchar(20) DEFAULT NULL COMMENT '本条记录的最新解析时间，方便查看记录什么时候解析的， 每个 data table 都必须有，直接解析为时间格式字符串，可以避免客户端再解析',
  `download_task_data_parse_time_number` bigint(100) DEFAULT NULL COMMENT '本条记录的最新解析时间，方便查看记录什么时候解析的，为数字格式， 每个 data table 都必须有，直接解析为，可以避免客户端再解析',
  `download_task_id` bigint(20) DEFAULT NULL COMMENT '本条记录对应的下载任务 id， 每个数据表都有这个字段， 便于分析解析不成功的情况',
  `download_task_level` int(11) DEFAULT NULL COMMENT '本条记录对应的下载任务 task_level， 每个数据表都有这个字段， 便于分析解析不成功的情况',
  `download_task_useless_content_page` tinyint(1) DEFAULT NULL COMMENT '本条记录该数据是否是有用的数据，比如数据解析比较完整，如果是则相对应的下载任务也设置为有用，方便以后下载，如果不是则相对应的下载任务也设置为无用，以后就不下载，每个数据表都有该字段',
  PRIMARY KEY (`id`)
