...
CREATE TABLE `data_search_log_detail` (
  ...
  PRIMARY KEY (`id`)
) ENGINE=MyISAM AUTO_INCREMENT=68 DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='记录用户所有搜索的详细信息，包括关键词，ip，时间，地区，在哪个范围搜索等信息';
...
) ENGINE=MyISAM AUTO_INCREMENT=54689 DEFAULT CHARSET=utf8 COLLATE=utf8_bin COMMENT='保存互联网抓取的食品信息';
...
) ENGINE=MyISAM AUTO_INCREMENT=164039 DEFAULT CHARSET=utf8 COLLATE=utf8_bin;
...
CREATE TABLE `site_url_parse` (
  ...
  `url_xpath_charactor` varchar(1000) COLLATE utf8_unicode_ci DEFAULT NULL COMMENT '网页链接所在页面区域块的 xpath 的特征字符串，xpath 里面有这些特征字符串的网页链接都是正确的链接，该字段为空则代表xpath非特征字符串之外的所有链接都是正确的',
  `not_url_xpath_charactor` varchar(1000) COLLATE utf8_unicode_ci DEFAULT NULL COMMENT '网页链接所在页面区域块的 xpath 的非特征字符串，链接元素 xpath 里面有这些特征字符串之一的就认为是不正确的链接',
